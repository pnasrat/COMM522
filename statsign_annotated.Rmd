---
title: "Statistical significance -- slightly annotated"
author:
  - Yph Lelkes, Comm 522
  - Pris Nasrat
  - Xinyi Wang
...
Significant in research can be seen as a standard to determine if your data approves your hypothesis or answers your research questions.
Substantive significance only focuses on the oberserved effect and whether it passess the standard to be meaningful,
Statistical significance uses different elements of the observed effect (coefficient, relationship etc) to determine if the results are to be considered important
...
```{r}
# Note text of question probably should say subtract mean of v as else we're operating on each elements of the vector vector
(10 - mean(v))/sd(v)
```
...
| Example                                                   | Statistic | Unit of Analysis | Concepts/Variables |
|-----------------------------------------------------------|-----------|------------------|--------------------|
| Mean years of education for working-age adults            | Years Ed |  Mean years       | Working-age, adult |
| Proportion of university graduates                        | People and graduation status | Ratio                 | graduation status                    |
| The male--female mean difference in annual income         |           |                  |                    |
| The correlations between education, sex, and income       |           |                  |                    |
| The causal effect of a university degree on annual income |           |                  |                    |
...
A data point may be considered unusual when a data is "too far away", at least 2 standard deviation from the mean?
...
We consider a statistic "significant" when most variation in the repeated samples from a population draw is explained, implying that the result is not likely due to chance but some factor of interest.
...
#  the tidy(verse) way!
# PN moved this import to initial block that doesn't get included in the knit to make for cleaner output
#library(tidyverse)
nba <- nba %>%
  filter(!is.na(FT.))
...
```{r}
# Note lab error here text says ten (10) observations, code says 5!
...
mean(s1)
sd(s1)
# Obviously this changes with each random sampling and given the small sample size we'd expect a lot of deviation from Mu and sigma
# in one case we saw:
# one variable is slightly far off from the others
```
...
```{r}
s1.se <- sd(s1)/sqrt(10)
s2.se <- sd(s2)/sqrt(100)
```

***Q10*** Calculate the 95 percent confidence interval for s1 and s2. Describe the confidence intervals in your own words.

```{r}
ci1_lower <- mean(s1) - (2 * s1.se)
ci1_upper <- mean(s1) + (2 * s1.se)
ci1_lower
ci1_upper
ci2_lower <- mean(s2) - (2 * s2.se)
ci2_upper <- mean(s2) + (2 * s2.se)
ci2_lower
ci2_upper
```
...
```{r}
# Visualize dist100 for convenience
df <-data.frame(dist100)
df %>%
  ggplot(aes(x = as.numeric(row.names(df)) ,y = dist100))  +
  geom_line()
```
...
# Note this 
length(dist100)
length(dist100[dist100 > (mean(nba$FT.) + 3*sd(dist100))]) + length(dist100[dist100 < (mean(nba$FT.) - 3*sd(dist100))])
length(dist100[dist100 > (mean(nba$FT.) + 2*sd(dist100))]) + length(dist100[dist100 < (mean(nba$FT.) - 2*sd(dist100))])
length(dist100[dist100 > (mean(nba$FT.) + 1*sd(dist100))]) + length(dist100[dist100 < (mean(nba$FT.) - 1*sd(dist100))])
length(dist100[dist100 <= (mean(nba$FT.) + 1*sd(dist100))]) + length(dist100[dist100 < (mean(nba$FT.) - 1*sd(dist100))])
```
...
We would use the first 50% quantile if onl looking at upper end of the distribution
We could use the last 50% quantile if only looking at lower end of the distribution
...
# the tidy way
nba <- nba %>%
  mutate(young1 = ifelse(Age <= 27, 1, 0))
...
According to the significant result, we reject the null hypothesis that old players ans new players are equally good at free throws.
